...
from ops import compute_box_distance,compute_attention_mask, generate_patch_grid, normalize_boxes
...
class CustomMultiheadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None, need_weights=False):
        ...
        q_proj_w = self.in_proj_weight[:self.embed_dim].clone()
        k_proj_w = self.in_proj_weight[self.embed_dim:2*self.embed_dim].clone()
        v_proj_w = self.in_proj_weight[2*self.embed_dim:].clone()

        q_proj_b = self.in_proj_bias[:self.embed_dim].clone()
        k_proj_b = self.in_proj_bias[self.embed_dim:2*self.embed_dim].clone()
        v_proj_b = self.in_proj_bias[2*self.embed_dim:].clone()

        query = query.permute(1, 0, 2)
        key = key.permute(1, 0, 2)
        value = value.permute(1, 0, 2)
        # 1️⃣ Project Q, K, V
        q = F.linear(query, q_proj_w, q_proj_b).view(B, N, self.num_heads, head_dim).transpose(1, 2)
        k = F.linear(key, k_proj_w, k_proj_b).view(B, N, self.num_heads, head_dim).transpose(1, 2)
        v = F.linear(value, v_proj_w, v_proj_b).view(B, N, self.num_heads, head_dim).transpose(1, 2)

        # 2️⃣ Compute attention scores
        attn_logits = (q @ k.transpose(-2, -1)) / (head_dim ** 0.5)

        # 3️⃣ Apply mask before softmax
        if attn_mask is not None:
            threshold = -1e6
            # Case 1: Apply multiplication where mask > threshold
            multiplicative_mask = attn_mask > threshold
            attn_logits = torch.where(multiplicative_mask, attn_logits * attn_mask, attn_logits + attn_mask)
            # attn_logits += attn_mask

        # 4️⃣ Apply softmax
        attn_probs = F.softmax(attn_logits, dim=-1)

        # 5️⃣ Compute attention output
        attn_output = (attn_probs @ v).transpose(1, 2).reshape(B, N, C)

        # 6️⃣ Final output projection
        attn_output = self.out_proj(attn_output)

        return attn_output.permute(1,0,2), attn_probs.mean(dim=1)  # Return both the final output & attention weights

class ResidualAttentionBlock(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: Optional[torch.Tensor] = None, adapter: bool=False, adapter_num_layers: int=1, pt_attn: str="none"):
        super().__init__()
        # self.attn = nn.MultiheadAttention(d_model, n_head)
        self.attn = CustomMultiheadAttention(d_model, n_head) if pt_attn != "none" else nn.MultiheadAttention(d_model, n_head)
        ...
        self.pt_attn = pt_attn
        # if pt_attn != 'none':
        #     self.pt_attn_head = 8
        #     self.pt_attn_module = nn.MultiheadAttention(embed_dim=d_model, num_heads=self.pt_attn_head)
        

    def attention(self, x: torch.Tensor, attn_mask: Optional[torch.Tensor] = None):
        mask = self.attn_mask if attn_mask is None else attn_mask
        if mask is not None:
            mask=mask.to(dtype=x.dtype, device=x.device)
        # self.attn_mask = self.attn_mask.to(dtype=x.dtype, device=x.device) if self.attn_mask is not None else None
        # return self.attn(x, x, x, need_weights=False, attn_mask=self.attn_mask)[0]
        return self.attn(x, x, x, need_weights=True, attn_mask=mask)
    ...
    def forward(self, x: torch.Tensor):
        ...
        ''' 
        x, prior, context, text_context, pair_prior = x
        
        
        if ( pair_prior is None):
            flag_pr_prior = False
        elif (len(pair_prior) > 1 and pair_prior[0] is None):
            flag_pr_prior = False
        else:
            flag_pr_prior = True
        ...
        if flag_pr_prior is True:
            if isinstance(pair_prior[0], list):
                pt_attn_mask = pair_prior[1]
                padded_sequences = pad_sequence(pair_prior[0], batch_first=True, padding_value=0)
                x = torch.cat((x.permute(1,0,2), padded_sequences), dim=1)

                attention_mask = (x.sum(dim=-1) != 0).int()

                float_mask = attention_mask[:, None, :].repeat(1, attention_mask.size(1), 1).float()
                attn_mask = float_mask.masked_fill(float_mask == 0, float('-1E9'))  # 将 0 的部分屏蔽为 -inf

                num_heads = self.attn.num_heads 
                batch_size, seq_len, _ = attn_mask.size()

                attn_mask = attn_mask.unsqueeze(1).expand(batch_size, num_heads, seq_len, seq_len)  # (batch_size, num_heads, seq_len, seq_len)
                # attn_mask = attn_mask.reshape(batch_size * num_heads, seq_len, seq_len) 

                if pt_attn_mask is not None:
                    pr_len = padded_sequences.shape[1]
                    ### maybe can combine the object oriented masks with the CLIP ViT self attention
     
                    attn_mask = attn_mask.contiguous()
                    ## pt attention to image tokens
                    attn_mask[:, :,-pr_len:, 1:-pr_len] = pt_attn_mask.unsqueeze(1).expand(batch_size, num_heads, pr_len, attn_mask.shape[-1]-pr_len-1)   ## .repeat_interleave(num_heads, dim=0).clone().contiguous()
                    ## pt attention to other pts
                    attn_mask[:, :,-pr_len:, -pr_len:] = pt_attn_mask.min(dim=-1, keepdim=True)[0].unsqueeze(1).repeat(1, num_heads, 1, pr_len)
                    attn_mask[:, :,-pr_len:, 0] = pt_attn_mask.min(dim=-1, keepdim=True)[0].unsqueeze(1).repeat(1, num_heads, 1, 1).squeeze(-1)

                pair_prior = (padded_sequences.permute(1,0,2), attn_mask)
                x = x.permute(1, 0, 2)  # (max_seq_len, batch_size, 768)
            else:
                x = torch.cat((x, pair_prior[0]), dim=0)
                attn_mask = pair_prior[1]
            pr_len = pair_prior[0].shape[0]
      
        else:
            attn_mask = None

        tempa = self.attention(self.ln_1(x), attn_mask)
        # tempa = self.attention(self.ln_1(x))

        x = x + tempa[0]  
        x = x + self.mlp(self.ln_2(x))  
        if flag_pr_prior is True:
            pair_prior = (x[-pr_len:], attn_mask)
            x = x[:-pr_len]
        if context is not None:
            tempb = tempa[1]
            if pair_prior is not None:
                tempb = tempb[:,:, :-pr_len]   ### avg attention
                # tempb = tempb[:,:-pr_len, :-pr_len]  ### CLIP original attention
                # tempb = tempb[:,-pr_len:, :-pr_len]  ### added learnable prompt attention

        else:
            tempb = None
            
        return (x, prior, tempb, text_context, pair_prior)

class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: Optional[torch.Tensor] = None, adapter: bool=False, adapter_layers: List=[i for i in range(24)], adapter_num_layers: int=1, pt_attn: str="none"):
        super().__init__()
        self.width = width
        self.layers = layers
        self.resblocks = nn.Sequential(*[ResidualAttentionBlock(width, heads, attn_mask, adapter=((_ in adapter_layers) and adapter), adapter_num_layers=adapter_num_layers, pt_attn = pt_attn) for _ in range(layers)])

    def forward(self, x: torch.Tensor, prior=None, context=None, text_context = None, pair_prior = None):
        # return self.resblocks((x,prior))[0]
        return self.resblocks((x, prior, context, text_context, pair_prior))

class VisionTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int, use_adapter: bool=True, adapter_layers: List=[_ for _ in range(24)], adapter_num_layers: int=1, pt_attn: str="none", pt_learn: int=0, pt_lyr: int = 0):
        super().__init__()
        self.input_resolution = input_resolution
        self.output_dim = output_dim
        ...
        self.transformer = Transformer(width, layers, heads, adapter=use_adapter, adapter_layers=adapter_layers, adapter_num_layers=adapter_num_layers, pt_attn = pt_attn)

        self.ln_post = LayerNorm(width)
        self.proj = nn.Parameter(scale * torch.randn(width, output_dim))
        self.patch_size = patch_size
        if pt_learn > 0:
            ctx_dim = 768 if input_resolution == 224 else 1024
            self.ctx_vectors = torch.empty(pt_learn, ctx_dim, dtype=dtype)
            nn.init.normal_(self.ctx_vectors, std=0.02)
            self.pt_lyr = pt_lyr
            self.pt_positional_embedding = nn.Parameter(scale * torch.randn(pt_learn, 1))
        self.pt_attn = pt_attn  
    ...
    def forward(self, x: torch.Tensor, prior=None, context = None, text_context = None, pair_prior = None):
        ...
        x = self.transformer(x,prior, context=context, text_context = text_context, pair_prior = pair_prior)
        paired_tokens= x[4]

        ##### original attention map visualization
        if context is not None:
            attn_qk = x[2][:,:,1:].unflatten(2, (self.input_resolution//self.patch_size,
                                                self.input_resolution//self.patch_size))

            # normalize the attention mask
            # if x[2].shape[1]==x[2].shape[2]:
            img = Image.open(os.path.join("../data/hico_20160224_det/images/test2015", context)).convert('RGB')
            img_h, img_w = img.size[0], img.size[1]
            plt.subplots(nrows=1, ncols=1, figsize=(0.02 * img_h, 0.02 * img_w))

            # scale the image
            img_h, img_w = int(img.size[0] ), int(img.size[1] )
            plt.imshow(img, interpolation='nearest')
            plt.axis('off')
            attn_msk = torch.mean(attn_qk, dim=1).squeeze(0).cpu().float().numpy()

            mask = cv2.resize(attn_msk, (img_h, img_w))
            normed_mask = mask / mask.max()
            normed_mask = (normed_mask * 255).astype('uint8')
            plt.imshow(normed_mask, alpha=0.5, interpolation='nearest', cmap="jet")
            folder = "/mnt/disk2/qinqian/ADA-CM/checkpoints/vis_attn/test_mean_glb"
            if os.path.exists(folder) is False:
                os.makedirs(folder)
            plt.savefig(os.path.join(folder, context.split(".")[0]+"_attn.jpg"))
            plt.clf()
            # else:
            #     for idx in range((attn_qk).shape[1]):
            #         img = Image.open(os.path.join("../data/hico_20160224_det/images/test2015", context)).convert('RGB')
            #         img_h, img_w = img.size[0], img.size[1]
            #         plt.subplots(nrows=1, ncols=1, figsize=(0.02 * img_h, 0.02 * img_w))
            ...
            #         plt.imshow(img, interpolation='nearest')
            #         plt.axis('off')
            #         attn_msk = (attn_qk[:,idx,:]).squeeze(0).cpu().float().numpy()
            #         mask = cv2.resize(attn_msk, (img_h, img_w))
            #         normed_mask = mask / mask.max()
            #         normed_mask = (normed_mask * 255).astype('uint8')
            #         plt.imshow(normed_mask, alpha=0.5, interpolation='nearest', cmap="jet")
            #         folder = "/mnt/disk2/qinqian/ADA-CM/checkpoints/vis_attn/test_mean_local"
            #         if os.path.exists(folder) is False:
            #             os.makedirs(folder)
            #         plt.savefig(os.path.join(folder, context.split(".")[0]+"_"+str(idx)+"_attn.jpg"))
            #         plt.clf()
        ...
        if ( paired_tokens is None):
            flag_pr_prior = False
        elif (len(paired_tokens) > 1 and paired_tokens[0] is None):
            flag_pr_prior = False
        else:
            flag_pr_prior = True

        if flag_pr_prior is True:
            paired_tokens = self.ln_post(paired_tokens[0].permute(1, 0, 2))
            if self.proj is not None:
                paired_tokens = paired_tokens @ self.proj

        return x[:,0,:], x[:,1:,:].view(bs,h//self.patch_size, w//self.patch_size, -1).permute(0, 3, 1, 2), paired_tokens
    ...
    def __init__(self,
        ...
        if isinstance(vision_layers, (tuple, list)):
            ...
            self.visual = VisionTransformer(
                ...
                pt_attn = kwargs['pt_attn'],
                pt_learn=kwargs['pt_learn'],
                pt_lyr=kwargs['pt_lyr'],
            )
...
def build_model(state_dict: dict, use_adapter=True, adapter_pos='all', adapter_num_layers=1, pt_attn: str="none", pt_learn=0, pt_lyr=0):
    ...
    model = CLIP(
        ...
        adapter_layers=adapter_layers, adapter_num_layers=adapter_num_layers, pt_attn = pt_attn,
        pt_learn=pt_learn, pt_lyr=pt_lyr,
    )


    # for name, module in model.named_modules():
    #     if isinstance(module, nn.MultiheadAttention):
    #         new_attn = CustomMultiheadAttention(module.embed_dim, module.num_heads, module.dropout)
    #         new_attn.load_from_pretrained(module)
    #         setattr(model, name, new_attn)
    #         print(f"🔄 Replaced {name} with CustomMultiheadAttention")
    ...
    print('[INFO] missing_keys:', [ k for k in missing_keys if 'adaptermlp' not in k and 'pt' not in k])
