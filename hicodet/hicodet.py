        ...
        try:
            ...
            for anno in f:
                for hoi in anno['hoi_annotation']:
                    num_anno[hoi['hoi_category_id']] += 1
            self._idx = idx
            self._num_anno = num_anno
            self._anno = f
            self._filenames = [temp['file_name'] for temp in f]
            self._image_sizes = [[512, 512] for i in range(len(f))]
            with open("./hicodet/instances_train2015.json", 'r') as f1:
                anno_hicodet = json.load(f1)
            self._class_corr = anno_hicodet['correspondence']
            self._empty_idx = anno_hicodet['empty']
            self._objects = anno_hicodet['objects']
            self._verbs = anno_hicodet['verbs']
